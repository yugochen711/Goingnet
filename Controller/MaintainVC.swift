...
class MaintainVC: BaseVC {
    ...
    private func readDB(upload: String) {
        var dataSource = [MaintainDataSource]()
        self.readDB(str: "SELECT * FROM goingnet WHERE type = '保養' AND upload_status = '\(upload)'") { (respone, err) in
            if var data = respone {
                //排序 最新日期開始
    ...
    @objc private func detailTopButton(_ sender: UIButton) {
        ...
        case "上傳", "刪除":
            let maintainUploadVC = Maintain_Storyboard.instantiateViewController(withIdentifier: "MaintainUploadVC") as! MaintainUploadVC
            maintainUploadVC.pageType = sender.currentTitle ?? ""
            self.navigationController?.pushViewController(maintainUploadVC, animated: true)
        default:
            break
        }
    }
...
extension MaintainVC {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        let currentOffset = scrollView.contentOffset.y
        add_Btn.alpha = 100 - currentOffset
    }
}
extension MaintainVC: UISearchBarDelegate {
...
extension MaintainVC: TopBarViewDelegate {
    func selectType(type: String) {
        upload = type
        readDB(upload: upload)
    }
}
extension MaintainVC: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.start_label.text = "保養開始 : \(searchDataSource[indexPath.section].data[indexPath.row].startDate ?? "")"
        cell?.end_label.text = "保養結束 : \(searchDataSource[indexPath.section].data[indexPath.row].endDate ?? "")"
...
extension MaintainVC: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        categorySecondVC.pageType = "Maintain"
        categorySecondVC.name = searchDataSource[indexPath.section].data[indexPath.row].name ?? ""
        categorySecondVC.group_id = searchDataSource[indexPath.section].data[indexPath.row].group_id ?? ""
        categorySecondVC.maintain_id = searchDataSource[indexPath.section].data[indexPath.row].maintain_id ?? ""
        categorySecondVC.startDate = searchDataSource[indexPath.section].data[indexPath.row].startDate ?? ""
        categorySecondVC.endDate = searchDataSource[indexPath.section].data[indexPath.row].endDate ?? ""
